...
if( isset($_POST['submit'])  ) {
	...
	$notetitle2=$_POST['notetitle3'];
	$notice2=$_POST['notice3'];
	
	$id=$_POST['nid3'];
	
	$sql = mysqli_query($conn,"update notice3 set notetitle3='$notetitle3',notice3='$notice3' where nid3='$id'") or die(mysqli_error($conn));
	
	if($sql) {
		
		echo "<script type='text/javascript'>alert('Notice-2 updated successfully!!..');</script>";
					  echo "<script>document.location='manage_notice2.php'</script>";
	} else {
		echo "<script type='text/javascript'>alert('error while updating Notice details!!..');</script>";
					  echo "<script>document.location='manage_notice2.php'</script>";
	} 
                            ...
                            <div class="card-body">
                                ...
                                <form class="new-added-form" method="post" enctype="multipart/form-data">
                                    <div class="row">
                                        ...
                                        <?php
                                        $sql=mysqli_query($conn,"select * from notice3 where nid3='$id'");
				while($row=mysqli_fetch_array($sql)) 
				{
				?>
                <input type="hidden" class="form-control" name="nid3" value="<?php echo $id; ?>">
												<label>Notice Title</label>
                                                <input type="text" class="form-control" placeholder="Notice Title" name="notetitle3" required value="<?php echo $row['notetitle3']; ?>">
                                                
                                            </div>
							
                                            <div class="col-12-xxxl col-lg-6 col-12 form-group">
												<label>Add Notice</label>
                                                <input type="text" class="form-control" placeholder="Add Notice" name="notice3" required value="<?php echo $row['notice3']; ?>">
