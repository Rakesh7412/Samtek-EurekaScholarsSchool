...
if( isset($_POST['login'])  ) {
 ...
 if ($conn->connect_error) {
        ...
        $ret=mysqli_query( $conn, "SELECT * FROM userslogin WHERE email='$email' and Password='$password' and cat='gate'") or die("Could not execute query: " .mysqli_error($conn));
        ...
        if(!$row) {
           ...
           echo "<script>document.location='gatelog.php'</script>";
        }
        else {
...
<body>
  ...
  <center>
  <h1 style="color:#C82128;">Welcome ESS</h1></center>
<div class="login-box">
  <h2>Gate Pass Login</h2>
