...
if( isset($_POST['submit'])  ) {
	...
	$sql = mysqli_query($conn,"INSERT INTO tblsubjectcombination(ClassId,SubjectId,status)VALUES ('$class','$subject','$status')") or die(mysqli_error($conn));
	
	if($sql) {
		
		echo "<script type='text/javascript'>alert('Combination added successfully!!..');</script>";
					  echo "<script>document.location='all_subject_combination.php'</script>";
	} else {
										...
										<label>Class</label>
                                            ...
                                            <div class="col-xl-4 col-lg-6 col-12 form-group">
													...
													<div class="col-sm-18">
														...
														<option value="">Please select</option>
														<?php 
														$sql=mysqli_query($conn,"select * from tblsubjects");
														while($row=mysqli_fetch_array($sql)) {
														?>
													<option value="<?php echo $row['id']; ?>"><?php echo $row['SubjectName']; ?>-<?php echo $row['SubjectCode']; ?></option><?php } ?>
												</select>
                                        </div>
									</div>
